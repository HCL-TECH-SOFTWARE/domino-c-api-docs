...
**NSFGetFirstLogToArchive** **- Get the
first log file waiting to be archived.**
...
STATUS
LNPUBLIC **NSFGetFirstLogToArchive(**  
...
This function
checks from the start of the list to determine if a log file is waiting to be
archived.  Whether the log file is waiting to be archived or not, it returns
the path to the log file, as well as the LogID and log sequence number to
