...
**ldap\_next\_entry** **- Retrieve
the next entry from a search result chain.**
...
This
function is used to step through and retrieve the list of entries from a search
result chain.
...
#define
ldap\_next\_entry(ld, entry)    ND\_ldap\_next\_entry((ld), (entry))
...
entry  -  The entry returned by a previous call to ldap\_first\_entry or
ldap\_next\_entry.  
...
NULL  - When no more entries or references exist in the result set to be
returned or if an error occurs while stepping through the entries or
references, in which case the error parameters in the session handle ld will be
