...
**NLS\_isarith** **- Tests if
a character is an arithmetic operator (+, -, =, etc.).**
...
This
function will return NLS\_SUCCESS if the character is considered an arithmetic
operator, or an error code if not.  Note that period (.) and comma (,) are
considered arithmetic operators, since these characters are used within numbers
as the thousands separator and the decimal separator.
...
(routine)  -  NLS\_SUCCESS - The specified character is an arithmetic operator.  

NLS\_PROPNOTFOUND - The specified character is NOT an arithmetic operator.  
