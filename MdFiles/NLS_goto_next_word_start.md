...
**NLS\_goto\_next\_word\_start** **- Advances
a pointer to the start of the next word in a string.**
...
WORD
LNPUBLIC **NLS\_goto\_next\_word\_start(**  
...
Advances a
pointer to the start of the next word in a string, skipping over the rest of
the current word (if any) as well as any whitespace that precedes the next
word.
...
(routine)  -  The  number of bytes that were skipped to get to the start of the
