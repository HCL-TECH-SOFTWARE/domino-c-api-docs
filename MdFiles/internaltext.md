...
/\* "internaltext" designates STATUS codes used to indicate a
condition  

 passed between 2 subsystems, but NEVER displayed to a user. The text  

 associated with "internaltext" is NOT EVEN STORED in the
executables,  
...
#define    ERR\_QUEUE\_NOT\_EMPTY    PKG\_MISC+1  

    internaltext(ERR\_QUEUE\_NOT\_EMPTY,     "Deleting a non-empty
queue")  

#define ERR\_QUEUE\_EMPTY                  PKG\_MISC+2  

    internaltext(ERR\_QUEUE\_EMPTY,         "No more entries to
dequeue")
